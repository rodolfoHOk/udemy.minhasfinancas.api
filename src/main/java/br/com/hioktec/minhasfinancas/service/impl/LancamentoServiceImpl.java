...
public class LancamentoServiceImpl implements LancamentoService{
	...
	public List<Lancamento> buscar(Lancamento lancamentoFiltro) {
		...
		/* sem api Example do spring data seria assim:
	...
	public void validar(Lancamento lancamento) {
		...
		if(lancamento.getMes() == null || lancamento.getMes() < 1 || lancamento.getMes() > 12) {
			throw new RegraNegocioException("Informe um Mês válido (número inteiro).");
		}
		
		if(lancamento.getAno() == null || lancamento.getAno().toString().length() < 4) {
			throw new RegraNegocioException("Informe um Ano válido (número inteiro com 4 casas).");
		}
		...
		if(lancamento.getValor() == null || lancamento.getValor().compareTo(BigDecimal.ZERO) < 1) { // 1 quer dizer maior, 0 igual, -1 menor
			throw new RegraNegocioException("Informe um Valor válido (número positivo).");
		}
		
		if(lancamento.getTipo() != TipoLancamento.DESPESA && lancamento.getTipo() != TipoLancamento.RECEITA) {
